...
**`POST /api/migrate` (added 2026-08-27) applies `db/schema.sql` for you.** API-key guarded, and safe to call repeatedly — the whole file is `IF NOT EXISTS`, so re-running is a no-op on anything already present. It returns the resulting `purchases` column list so you can confirm the change landed. **Call it after any schema.sql change instead of reaching for psql.**

Historically: **no migration mechanism existed.** Confirmed by grepping the whole repo for `migrate`/`schema.sql` outside `node_modules` — zero hits. `schema.sql` is a flat, idempotent (`CREATE TABLE IF NOT EXISTS` / `ADD COLUMN IF NOT EXISTS`) file meant to be applied by hand. **Whenever `schema.sql` changes, it must be run manually against production** — `psql "<DATABASE_URL from Render's Connect tab>"`, or paste the new `ALTER TABLE` line directly into a `psql` session. Nothing in `index.js`/`db/pool.js`/`db/seed.js` runs it automatically, and there's no drift-detection — a column that exists in `schema.sql` but was never actually applied to prod will make the corresponding route throw at insert time with a plain Postgres "column does not exist" error, not a friendly message.
...
**Owner Home now polls every 15s while it's the active page (added 2026-08-04).** `inventory_items` only changes from sales happening in the OTHER app (the WhatsApp bot's kitchen/cashier, a separate repo/process) — this app has no way to know that happened without polling; the owner reported a drink-can "ready" count that looked frozen rather than reflecting live sales, because nothing ever re-fetched after the initial page load. `go(id)` now starts `setInterval(loadOwnerHome, 15000)` (stored in `ownerHomePoll`) when navigating TO `'owner-home'`, and clears it on every other navigation — do not add a second polling loop elsewhere; the three old call sites that used to manually call `loadOwnerHome()` right after `go('owner-home')` were removed as redundant since `go()` now does it. If a similar "reflect the other app's live state" need comes up on another page, follow this same start-on-enter/clear-on-leave pattern rather than polling unconditionally in the background.

**Layout: `.protein-row` and the bag-cut rows (`calcSeasoning`) must stay `flex-wrap`-friendly.** Both are flex rows with a text label on the left and a short value on the right and NO wrap by default — fine for short labels, but the Stock section's longer strings (e.g. `"Color (Cherry/Pineapple/Grape/Orange)"` as a label, or a long `"Raw: X · Ready: Y"` value) overflowed the card and caused page-level horizontal scroll on narrow phones (owner report, 2026-08-04: "scrolling ugly"). Fixed with `flex-wrap:wrap` + `min-width:0` on the shrinking side + `flex-shrink:0` on the side that should stay fixed-width, plus a defensive `html,body{overflow-x:hidden}` + `body{max-width:100vw}` as a last-resort safety net. **If you add a new flex row with variable-length text on either side, add `flex-wrap`/`min-width:0` from the start** — this class of bug is easy to reintroduce and only shows up on an actual narrow phone, not in a wide desktop preview.
...
## Hours that never showed up on Monday (owner-reported, fixed 2026-09-22)
*"He punched in a few days and they did not pop up on the monday hours control."* Two independent causes, both in this file, plus one in the bot:

1. **An `open` entry was excluded from the query, so it was invisible — not zero, absent.** `hoursForStaff` selected only `'closed','approved'`, so a shift someone forgot to close vanished from `/hours/:name` AND from the owner's `/timesheet` with nothing to show it had existed. Open entries are now always returned in `entries` and counted in **`openCount`**; they still contribute **0 hours**, because there is no end time to count to. The bot renders them as "⚠️ NEVER CLOCKED OUT".
2. **`weekBounds` was computed in UTC** and documented as an acceptable limitation. It was not: the week ended Sunday 23:59 UTC = **Sunday 19:59 Aruba**, and the shift starts 18:30, so a Sunday punch clocked in at 20:00 or later landed in the FOLLOWING week. Now Monday 00:00 → Sunday 23:59 **Aruba** (fixed -04:00, no DST). **This shifts hours between weeks once, on the week it deploys** — expected, and correct.
3. (In the bot) the 3am auto-clockout that closes these entries only fired in the single minute 07:00 UTC. Now the whole 07:00 hour.

**New: `GET /api/time/open`** — every punch with no clock-out, with how long it has been open. Nothing could answer that question before, which is why these sat unnoticed until payroll. The bot exposes it as the admin command **`open punches`**.

**Rule of thumb this cost us:** a record that is filtered out of a view is not "missing data", it is a silent lie about the hours someone worked. Filter to change the TOTAL, never to hide the ROW.
